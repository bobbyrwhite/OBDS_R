...
> Answer: number of rows & columns, names of rows & columns & types of data
...
iris_for_ggplot <- iris_raw %>%
  ...
  select(species, sepal_area) %>%
  arrange(desc(sepal_area)) %>%
  ggplot(aes(x = sepal_area, color = species, fill = species)) +
  geom_histogram() +
...
iris_long <- iris_raw %>% 
  pivot_longer(is.numeric, names_to = "variable")
  iris_long
```
...
iris_long2 <- iris_raw %>%
  mutate( flower_ID = paste0("flower", seq(1, nrow(iris_raw))))%>%
  pivot_longer(is.numeric, names_to = "variable")
...
iris_raw_new <- iris_long2 %>%
  pivot_wider(id_cols = flower_ID, names_from = variable)
...
```{r}
library(readxl)
read_excel("/t1-data/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx")
excel_sheets("/t1-data/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx")
read_excel("/t1-data/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet = "exprs")

exprs_assay <-tibble(read_excel("/t1-data/project/obds/shared/resources/2_r/r_tidyverse/EH2011.xlsx", sheet = "exprs"))
...
exprs_assay_long <- exprs_assay %>% 
  pivot_longer(is.numeric, names_to = "sample")
...
```{r}
ggplot(exprs_assay_long, aes(x = value, color = gene, fill = gene)) +
  geom_histogram() +
