...
# fixed needs to be false because NN in Ebox represents any nucleotide
...
gtf_ACTB_transcripts <- subset(gtf_ACTB, type == "transcript")
...
# length tells you the length of the list, lengths tells you the length of each element in the list -i.e. number of exons in each transcript
...
mapIds(org.Hs.eg.db, keys = human_symbols, column = "GO", keytype = "SYMBOL")
```
...
```{r}
gene_metadata <- gene_metadata <- read.csv("/project/obds/shared/resources/3_r_stats_genomics/r_bioconductor/summarized_experiment/gene_metadata.csv", row.names = "X")
head(gene_metadata)
```
